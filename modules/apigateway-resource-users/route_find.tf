...
data "archive_file" "lambda_package_users_find_by_id" {
  type        = "zip"
  source_dir  = "${path.root}/lambdas/core/users-find-by-id"
  output_path = "${path.root}/resources/core/users-find-by-id/index.zip"
}
...
resource "aws_apigatewayv2_integration" "lambda_handler_users_find_by_id" {
  ...
  depends_on       = [aws_apigatewayv2_api.main, aws_lambda_function.html_lambda_users_find_by_id]
}
